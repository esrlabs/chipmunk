...
impl NestedSearch {
    ...
    fn render_row(
        ...
        let enter_pressed =
            ui.input_mut(|input| text_focused && input.consume_key(Modifiers::NONE, Key::Enter));
        ...
        ui.horizontal(|ui| {
            ...
            ui.add_enabled(
                false,
                buttons::bottom_panel_icon(RichText::new(icons::regular::ARROW_UP).size(14.0)),
            )
            .on_disabled_hover_text("Previous match");
            ui.add_enabled(
                false,
                buttons::bottom_panel_icon(RichText::new(icons::regular::ARROW_DOWN).size(14.0)),
            )
            .on_disabled_hover_text("Next match");

            close_requested =
                buttons::bottom_panel_icon(RichText::new(icons::regular::X).size(14.0))
                    .ui(ui)
                    .on_hover_text("Close")
                    .clicked();
        });
...
mod tests {
    ...
    fn invalid_submission_warns_without_replacing_filter() {
        ...
        let widget = NestedSearch {
            query: "(".to_owned(),
            ..NestedSearch::default()
        };
