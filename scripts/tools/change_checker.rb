...
module ChangeChecker
  ...
  # the previous run.
  def self.create_changelist(path, omissions = [])
    FileUtils.mkdir_p Paths::CHECKLISTS, verbose: true
    checklist_path = ChangeChecker.checklist_path(path)
  ...
  def self.changes?(path)
    old_checklist_file = ChangeChecker.checklist_path(path)
  ...
  def self.reset(path, omissions = [])
    if @first_run
      ChangeChecker.create_changelist(path, omissions)
    else
      old_checklist_file = ChangeChecker.checklist_path(path)
      old_fingerprint = eval(File.read(old_checklist_file))
      ChangeChecker.create_changelist(path, old_fingerprint[:omissions])
    end
  end

  def self.checklist_path(path)
    "#{Paths::CHECKLISTS}/.#{ChangeChecker.element_name(path)}_checklist.chk"
  end
