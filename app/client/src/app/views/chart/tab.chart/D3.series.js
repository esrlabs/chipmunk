...
var D3Controller = (function () {
    function D3Controller(selector, onSelect) {
    ...
    D3Controller.prototype.resize = function () {
        if (this.size.height > 0 && !isNaN(this.size.height) && this.size.width > 0 && !isNaN(this.size.width) && !this.ready) {
            this.size.margin = { top: 10, right: 20, bottom: 200, left: 20 };
            this.size.margin2 = { top: 460, right: 20, bottom: 20, left: 20 };
            this.size.width = +this.svg.attr("width") - this.size.margin.left - this.size.margin.right;
    ...
    D3Controller.prototype.destroy = function () {
    ...
    D3Controller.prototype.onData = function (src) {
        var _this = this;
        this.destroy();
        if (this.init()) {
            this.data = src.data;
            ...
            Object.keys(this.data).forEach(function (GUID) {
                _this.createMainLine(GUID);
            });
    ...
    D3Controller.prototype.createMainLine = function (GUID) {
        var serie = this.tools.focus.selectAll('.serie' + GUID).
            data([this.data[GUID]]).
            enter().
            append('g').
    ...
    D3Controller.prototype.createZoomLine = function (GUID) {
        var serie = this.tools.context.selectAll('.serie.' + GUID).
            data([this.data[GUID]]).
            enter().
            append('g').
            attr('class', 'serie ' + GUID);
    ...
    D3Controller.prototype.labelsRender = function (GUID, serie) {
        var label = serie.selectAll('.label.' + GUID).
            data(function (d) { return d; }).
            enter().
            append("g").
            attr("class", 'label ' + GUID).
            attr("transform", function (d, i) { return "translate(" + this.axis.x(d.datetime) + "," + this.axis.y(d.value) + ")"; }.bind(this));
        label.append("text").
    ...
    D3Controller.prototype.labelsUpdate = function () {
        this.tools.focus.selectAll('.label').
            attr("transform", function (d, i) {
            return "translate(" + this.axis.x(d.datetime) + "," + this.axis.y(d.value) + ")";
        }.bind(this));
    };
    D3Controller.prototype.validateRange = function (range) {
        return range;
    };
    D3Controller.prototype.correctSelectionRect = function () {
        var selection = this.tools.context.select(".selection"), width = selection.attr("width");
        selection.attr("width", (width < SETTINGS.MINIMAL_SELECTION_WIDTH ? SETTINGS.MINIMAL_SELECTION_WIDTH : width));
    };
    D3Controller.prototype.onViewPort = function (incomeSelection) {
        var _this = this;
        if (incomeSelection instanceof Array || (!this.d3.event.sourceEvent || this.d3.event.sourceEvent.type !== "zoom")) {
            var selection = incomeSelection instanceof Array ? incomeSelection : (this.d3.event.selection || this.axis.x2.range());
            this.axis.x.domain(selection.map(this.axis.x2.invert, this.axis.x2));
            Object.keys(this.data).forEach(function (GUID) {
                _this.tools.focus.select('.line.' + GUID).attr("d", _this.tools.area);
            });
            this.tools.focus.select(".axis--x").call(this.axis.xAxis);
            var rate = this.size.width / (selection[1] - selection[0]);
    ...
    D3Controller.prototype.zoomed = function () {
        ...
        if (this.d3.event !== null && (!this.d3.event.sourceEvent || this.d3.event.sourceEvent.type !== "brush")) {
            var transform = this.d3.event.transform, brushRange = null;
            this.axis.x.domain(transform.rescaleX(this.axis.x2).domain());
            Object.keys(this.data).forEach(function (GUID) {
                _this.tools.focus.select('path.line.' + GUID).attr("d", _this.tools.area);
            });
            this.tools.focus.select(".axis--x").call(this.axis.xAxis);
    ...
    D3Controller.prototype.goToPosition = function (selected) {
        var range = null, left = null, right = null;
...
//# sourceMappingURL=D3.series.js.map