...
import { SymlinkSupport } from './symlink';
...
async function findPSCoreWindowsInstallation(
	...
	const powerShellInstallBaseDir = paths.join(programFilesPath, 'PowerShell');
	...
	for (const item of await fs.promises.readdir(powerShellInstallBaseDir)) {
		...
		const exePath = paths.join(powerShellInstallBaseDir, item, 'pwsh.exe');
...
async function findPSCoreMsix({ findPreview }: { findPreview?: boolean } = {}): Promise<IPossiblePowerShellExe | null> {
	...
	const msixAppDir = paths.join(process.env['LOCALAPPDATA'], 'Microsoft', 'WindowsApps');
	...
	for (const subdir of await fs.promises.readdir(msixAppDir)) {
		if (pwshMsixDirRegex.test(subdir)) {
			const pwshMsixPath = paths.join(msixAppDir, subdir, 'pwsh.exe');
...
function findPSCoreDotnetGlobalTool(): IPossiblePowerShellExe {
	const dotnetGlobalToolExePath: string = paths.join(os.homedir(), '.dotnet', 'tools', 'pwsh.exe');
