...
/// The width between quick actions and recent sessions group frames.
const FRAME_GROUPS_GAP_WIDTH: f32 = 8.0;
const WIDE_LAYOUT_WIDTH: f32 =
    QUICK_ACTIONS_WIDE_WIDTH + FRAME_GROUPS_GAP_WIDTH + RECENT_SESSIONS_MAX_WIDTH;
const COMPACT_LAYOUT_WIDTH: f32 =
    QUICK_ACTIONS_COMPACT_WIDTH + FRAME_GROUPS_GAP_WIDTH + RECENT_SESSIONS_MAX_WIDTH;
...
impl HomeView {
    ...
    pub fn render_content(
        ...
        CentralPanel::default().show_inside(ui, |ui| {
            ...
            //   and recent sessions still stay capped at their maximum width.
            let available_height = ui.available_height();
            let show_wide_quick_actions = ui.available_width() >= WIDE_LAYOUT_WIDTH;
            let layout_width = if show_wide_quick_actions {
                WIDE_LAYOUT_WIDTH
            } else {
                COMPACT_LAYOUT_WIDTH.min(ui.available_width())
            };
            let recent_sessions_width = if show_wide_quick_actions {
                RECENT_SESSIONS_MAX_WIDTH
            } else {
                (layout_width - QUICK_ACTIONS_COMPACT_WIDTH - FRAME_GROUPS_GAP_WIDTH).max(0.0)
            };

            ui.with_layout(Layout::top_down(Align::Center), |ui| {
                ui.allocate_ui_with_layout(
                    vec2(layout_width, available_height),
