    ...
    pub fn new(with_storage_header: bool, source_path: Option<PathBuf>) -> Self {
        ...
        if let Some(source) = source_path {
            if let Ok(path) = source.into_os_string().into_string() {
                if let Some(statistics) = statistics::collect(path) {
...
pub mod statistics {
    ...
    pub fn collect(path: String) -> Option<DltStatistics> {
    ...
    /// The statistics-info on the DLT file.
    ...
    /// The Level distribution with DLT message indexes
    #[derive(Debug, Default, Clone)]
