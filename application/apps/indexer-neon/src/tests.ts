...
export function testCallConcatFiles(concatConfig: string, out: string, chunk_size: number) {
    ...
    let onNotification = (notification: INeonNotification) => {
        log.debug(
            'TTT: testCallConcatFiles: received notification:' + JSON.stringify(notification),
        );
    };
    measure({
...
export function testCheckFormatString(
    ...
    try {
        ...
        let onNotification = (notification: INeonNotification) => {
            log.debug(
                'testCheckFormatString: received notification:' + JSON.stringify(notification),
            );
        };
...
export function testDiscoverTimestampAsync(files: string[]) {
    ...
    try {
        ...
        let onNotification = (notification: INeonNotification) => {
            log.debug(
                'testDiscoverTimestampAsync: received notification:' + JSON.stringify(notification),
            );
        };
...
export function testConvertPcapToDlt(input: string) {
    ...
    try {
        ...
        let onNotification = (notification: INeonNotification) => {
            log.debug(
                'testConvertPcapToDlt: received notification:' + JSON.stringify(notification),
            );
        };
...
export function testIndexingAsync(inFile: string, outPath: string, chunkSize: number) {
    log.setDefaultLevel(log.levels.DEBUG);
    log.debug(
        `testIndexingAsync for ${inFile} (chunkSize: ${chunkSize}, type: ${typeof chunkSize})`,
    );
    const hrstart = process.hrtime();
    const bar = stdout.createProgressBar({ caption: 'index file', width: 60 });
    try {
        ...
        createGrabberAsync(path)
            .then((grabber) => {
                ...
                if (entry_cnt !== undefined) {
                    ...
                    log.info(
                        'Execution time for grabbing %d lines from index %d: %dus',
                        lines_to_grab,
                        start_line,
                        us,
                    );
