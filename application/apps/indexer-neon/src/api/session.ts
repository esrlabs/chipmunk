...
export class Session {
	...
	public init(): Promise<void> {
		return new Promise<void>((resolve, reject) => {
			...
			const channel = new RustSessionChannelConstructor();
			const computation = new SessionComputation(channel, this._uuid);
			subs.done = computation.getEvents().done.subscribe(() => {
				done();
			});
			subs.error = computation.getEvents().error.subscribe((err: Error) => {
				done(err);
			});
		});
	}
		...
		return new Promise((resolve, reject) => {
			...
			Promise.all([
				...
				(this._stream as SessionStream).destroy().catch((err: Error) => {
					this._logger.error(`Fail correctly destroy SessionStream due error: ${err.message}`);
				}),
				// Destroy search controller
				(this._search as SessionSearch).destroy().catch((err: Error) => {
					this._logger.error(`Fail correctly destroy SessionSearch due error: ${err.message}`);
				}),
			]).catch((err: Error) => {
