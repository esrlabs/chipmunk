...
export type TDltStatsEventObject =
    | TDltStatsEventConfig
...
export function dltStatsAsync(
    ...
    return new CancelablePromise<void, void, TDltStatsEvents, TDltStatsEventObject>(
...
export function exportDltFile(
    ...
    return new CancelablePromise<void, void, TDltFileAsyncEvents, TDltFileAsyncEventObject>(
        (resolve, reject, cancel, refCancelCB, self) => {
            try {
                ...
                // Create channel
                const channel = new RustExportFileChannel(
                    source,
                    sourceType,
                    targetFile,
                    params,
                ...
                // Create emitter
                const emitter: NativeComputationManager<ITicks> = new NativeComputationManager(
                    channel,
                );
                let chunks: number = 0;
                // Add listenters
                ...
                emitter.onStopped(() => {
                    log(
                        `we got a stopped event while saving dlt file (${sourceType}) with source ${source}`,
                    );
                    cancel();
                });
                emitter.onNotification((notification: INeonNotification) => {
...
export function indexDltAsync(
    ...
    return new CancelablePromise<void, void, TIndexDltAsyncEvents, TIndexDltAsyncEventObject>(
        (resolve, reject, cancel, refCancelCB, self) => {
            try {
                ...
                const emitter: NativeComputationManager<INeonTransferChunk> =
                    new NativeComputationManager(channel);
                let chunks: number = 0;
                // Add listenters
...
export function dltOverSocket(
    ...
    return new CancelablePromise<void, void, TDLTSocketEvents, TDLTSocketEventObject>(
        (resolve, reject, cancel, refCancelCB, self) => {
            log(`dltOverSocket: params: ${JSON.stringify(params)}`);
            try {
                socketConfig.udp_connection_info =
                    socketConfig.target === 'Udp'
                        ? {
...
function getDefaultIndexDltProcessingOptions(
    options: IIndexDltOptions | undefined,
): IIndexDltOptionsChecked {
...
export function pcap2dlt(
    ...
    return new CancelablePromise<void, void, TPcap2DltEvents, TPcap2DltsEventObject>(
        (resolve, reject, cancel, refCancelCB, self) => {
            try {
                // Create channel
                const channel = new RustDltPcapConverterChannel(pcapFilePath, outFilePath);
                // Create emitter
                const emitter: NativeComputationManager<void> = new NativeComputationManager(
                    channel,
                );
                let chunks: number = 0;
