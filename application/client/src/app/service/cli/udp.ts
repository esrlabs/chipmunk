    ...
    return new CancelablePromise(async (resolve, _reject) => {
        action(cli, request)
            .then((sessions: string[]) => {
                resolve(new Requests.Cli.Udp.Response({ sessions, error: undefined }));
            })
            .catch((err: Error) => {
                resolve(new Requests.Cli.Udp.Response({ sessions: undefined, error: err.message }));
            });
    });
}

export async function action(cli: Service, request: Requests.Cli.Udp.Request): Promise<string[]> {
