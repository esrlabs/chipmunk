...
export class ModifierProcessor {
    ...
    private _logger: Logger = scope.getLogger('ModifierProcessor');
    ...
    private _getApplicableModifiers(target: Owner, hasOwnStyles: boolean): Modifier[] {
        const applyTo: EApplyTo = (() => {
            if (target === Owner.Output) {
                return EApplyTo.output;
            }
            if (target === Owner.Search) {
