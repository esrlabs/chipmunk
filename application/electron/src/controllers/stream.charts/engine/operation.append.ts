...
export class OperationAppend extends EventEmitter {
    ...
    public perform( regExp: RegExp,
        ...
        const task = new CancelablePromise<IMatch[], void>((resolve, reject) => {
    ...
    public drop() {
        ...
        this._tasks.forEach((task: CancelablePromise<IMatch[], void>, id: string) => {
            this._logger.warn(`Dropping charting controller, while appending operation is still in progress. Current task "${id}" will be dropped`);
            task.break();
        });
        // Drop data
        this._tasks.clear();
