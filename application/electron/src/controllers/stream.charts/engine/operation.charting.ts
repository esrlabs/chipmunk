...
export class OperationCharting extends EventEmitter {
    ...
    public perform(regExp: RegExp, from: number | undefined, groups: boolean = false): CancelablePromise<IMatch[], void> {
        ...
        return new CancelablePromise<IMatch[], void>((resolve, reject) => {
            fs.exists(this._streamFile, (exists: boolean) => {
                ...
                // Create reader
                const reader: ReadStream = fs.createReadStream(this._streamFile, { encoding: 'utf8', start: from });
                // Create writer
