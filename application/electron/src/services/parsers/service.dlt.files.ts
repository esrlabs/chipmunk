...
class ServiceDLTFiles implements IService {
    ...
    private _tasks: Map<
        string,
        CancelablePromise<void, void, DLT.TDltStatsEvents, DLT.TDltStatsEventObject>
    > = new Map();
    ...
    public init(): Promise<void> {
        return new Promise((resolve, reject) => {
            Promise.all([
                ...
                ServiceElectron.IPC.subscribe(
                    IPCMessages.DLTStatsCancelRequest,
                    this._onDLTStatsCancelRequest.bind(this),
                ).then((subscription: Subscription) => {
                    this._subscription.DLTStatsCancelRequest = subscription;
                }),
                ServiceElectron.IPC.subscribe(
                    IPCMessages.DLTRecentTimeZoneRequest,
                    this._onDLTRecentTimeZoneRequest.bind(this),
                ).then((subscription: Subscription) => {
                    this._subscription.DLTRecentTimeZoneRequest = subscription;
                }),
            ])
                .then(() => {
                    resolve();
    ...
    private _onDLTStatsRequest(
        ...
        ServiceStreams.addProgressSession(taskId, 'Getting DLT stats', req.session);
        // return new Promise((resolve, reject) => {
        const hrstart = process.hrtime();
        this._logger.debug('calling _onDLTStatsRequest with params: ' + JSON.stringify(req));
        let stats: DLT.StatisticInfo | undefined;
        const task: CancelablePromise<void, void, DLT.TDltStatsEvents, DLT.TDltStatsEventObject> =
            indexer
                .dltStatsAsync(req.file)
                .then(() => {
                ...
                .on('config', (event: DLT.StatisticInfo) => {
                    ...
                    stats = event;
                    this._logger.debug(
                        `_onDLTStatsRequest: received config, contained non-verbose logs: ${event.contained_non_verbose}`,
                    );
                })
    ...
    private _onDLTStatsCancelRequest(
        request: IPCMessages.TMessage,
        response: (instance: IPCMessages.TMessage) => any,
    ) {
        ...
        if (task === undefined) {
            return response(
                new IPCMessages.DLTStatsCancelResponse({
                    id: req.id,
                    session: req.session,
    ...
    private _onDLTRecentTimeZoneRequest(
        request: IPCMessages.DLTRecentTimeZoneRequest,
        response: (instance: IPCMessages.DLTRecentTimeZoneResponse) => any,
    ) {
        const stored: IStorageScheme.IStorage = ServiceStorage.get().get();
        response(
            new IPCMessages.DLTRecentTimeZoneResponse({
                timezone: stored.recentDLTTimeZone === undefined ? '' : stored.recentDLTTimeZone,
            }),
        );
    }
}
