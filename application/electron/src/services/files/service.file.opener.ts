...
import * as Parsers from '../../controllers/files.parsers/index';
...
class ServiceFileOpener implements IService {
        ...
        return new Promise((resolve, reject) => {
            Promise.all([
                ServiceElectron.IPC.subscribe(
                    IPCMessages.FileOpenRequest,
                    this._ipc_FileOpenRequest.bind(this),
                ).then((subscription: Subscription) => {
                    this._subscriptions.FileOpenRequest = subscription;
                }),
                ServiceElectron.IPC.subscribe(
                    IPCMessages.FileListRequest,
                    this._ipc_FileListRequest.bind(this),
                ).then((subscription: Subscription) => {
                ...
                .then(() => {
                    ...
                    this._subscriptions.openLocalFile =
                        ServiceHotkeys.getSubject().openLocalFile.subscribe(
                            this._hotkey_openLocalFile.bind(this),
                        );
                    resolve();
                })
                .catch((error: Error) => {
    ...
    public destroy(): Promise<void> {
        return new Promise((resolve) => {
            ...
            Promise.all(
                Array.from(this._active.values()).map((task: AFileParser) => {
                    return task.abort();
                }),
            )
                .catch((error: Error) => {
                ...
                getParserForFile(file, parser)
                    .then((detected: IFileParserInfo) => {
                        ...
                        if (opts !== undefined) {
                            ...
                            this._getOptions(file, path.basename(file), instanceParser, stats.size)
                                .then((options: any) => {
                                    open(options);
                                })
                                .catch((getOptionsError: Error) => {
                                    reject(
                                        new Error(
                                            this._logger.error(
                                                `File "${file}" (${instanceParser.getAlias()}) will not be opened due error: ${
                    ...
                    .catch((gettingParserError: Error) => {
                        reject(
                            new Error(
                                this._logger.warn(
                                    `Fail to find parser due error: ${gettingParserError.message}`,
                                ),
                            ),
                        );
                    });
            ...
            ServiceElectron.IPC.request(
                ...
                .then((response: IPCMessages.RenderSessionAddResponse) => {
                    if (response.error !== undefined) {
                        this._logger.warn(
                            `Fail to add new session for file "${file}" due error: ${response.error}`,
                        );
                        return reject(new Error(response.error));
                    }
                    this.open(file, response.session)
                        .then(() => {
                            resolve();
                        })
                        .catch((openFileErr: Error) => {
                            this._logger.warn(
                                `Fail to open file "${file}" due error: ${openFileErr.message}`,
                            );
                            reject(openFileErr);
                        });
                })
                .catch((addSessionErr: Error) => {
                    this._logger.warn(
                        `Fail to add new session for file "${file}" due error: ${addSessionErr.message}`,
                    );
        ...
        const subscription = ServiceStreams.getSubjects().onSessionCreated.subscribe((session) => {
            ...
            this.open(filename, session.stream.guid).catch((err: Error) => {
                ServiceNotifications.notify({
                    message: this._logger.warn(
                        `Fail open file "${filename}" due error: ${err.message}`,
                    ),
    ...
    private _ipc_FileListRequest(
        ...
        Promise.all(
            ...
            .then((fileLists: IPCMessages.IFile[][]) => {
                response(
                    new IPCMessages.FileListResponse({
                        files: this._concatFileList(fileLists),
                    }),
                ).catch((error: Error) => {
                    this._logger.error(
                        `Fail to respond to files ${req.files} due error: ${error.message}`,
                    );
                });
            })
            .catch((error: Error) => {
    ...
    private _ipc_FileOpenRequest(
        request: IPCMessages.TMessage,
        response: (instance: IPCMessages.TMessage) => any,
    ) {
        const req: IPCMessages.FileOpenRequest = request as IPCMessages.FileOpenRequest;
        this.open(req.file, req.session, undefined, req.options)
            .then((result: IOpenFileResult) => {
                ...
                response(
                    new IPCMessages.FileOpenResponse({
                        stream: info as IPCMessages.IStreamSourceNew,
                    ...
                    fs.lstat(file, (lsErr, stats) => {
                        ...
                        if (stats.isFile()) {
                            ...
                            Promise.all([getParserForFile(file), isHidden(file)])
                                ...
                                .catch((error: Error) => {
                                    self._logger.warn(
                                        `Fail to indentify parser of ${file} due to error: ${error.message}`,
                                    );
                                    return resolved(
                                        allFiles.push({
                                            hasParser: false,
                            ...
                            return rejected(
                                new Error(
                                    self._logger.warn(
                                        `Fail to get file info of ${file} because it is neither a file nor a directory`,
                                    ),
                                ),
                            );
                        } else {
                            ...
                            return fs.readdir(file, (err, files) => {
                                if (err) {
                                    self._logger.warn(
                                        `Fail to list files of directory ${file} due to error: ${err.message}`,
                                    );
                                    return resolved([]);
                                } else {
                                    Promise.all(
                                        files.map((subFile: string) => {
                                        ...
                                        .catch((error: Error) => {
                                            rejected(
                                                new Error(
                                                    self._logger.warn(
                                                        `Fail to list files of directory ${file} info due to error: ${error.message}`,
                                                    ),
                                                ),
                                            );
                                        });
            ...
            listAllFiles(startFile)
                ...
                .catch((error: Error) => {
                    reject(
                        new Error(
                            self._logger.warn(
                                `Fail to list files of directory ${startFile} info due to error: ${error.message}`,
                            ),
                        ),
                    );
                });
    ...
    private _getOptions(
        ...
        return new Promise((resolve, reject) => {
            ServiceElectron.IPC.request(
                new IPCMessages.FileGetOptionsRequest({
                    fileName: fileName,
                ...
                IPCMessages.FileGetOptionsResponse,
            )
                .then((response: IPCMessages.FileGetOptionsResponse) => {
                    if (!response.allowed) {
    ...
    private _directReadWrite(
        file: string,
        sessionId: string,
        parser: AFileParser,
        options: { [key: string]: any },
        trackingId: string,
    ): Tools.CancelablePromise<number, void> {
    ...
    private _onSessionClosed(guid: string) {
        ...
        task.abort().then(() => {
            this._logger.debug(
                `Session "${guid}" is closed. Task "${task.getAlias()}" is aborted.`,
            );
        });
    }
}
