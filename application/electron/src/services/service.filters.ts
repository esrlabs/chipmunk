...
class ServiceFilters implements IService {
    ...
    public openFilters(session: string, file?: string) {
        ServiceElectron.IPC.send(new ServiceElectron.IPCMessages.FiltersOpen({ session: session, file: file })).catch((error: Error) => {
