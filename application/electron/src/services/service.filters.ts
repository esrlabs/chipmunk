...
class ServiceFilters implements IService {
    ...
    private _ipc_onFiltersSaveRequest(message: IPCElectronMessages.TMessage, response: (message: IPCElectronMessages.TMessage) => Promise<void>) {
        ...
        dialog.showSaveDialog({
            ...
            fs.writeFile(results.filePath, content, (error: NodeJS.ErrnoException | null) => {
                if (error) {
                    this._logger.warn(`Error during saving filters into file "${results.filePath as string}": ${error.message}`);
                }
