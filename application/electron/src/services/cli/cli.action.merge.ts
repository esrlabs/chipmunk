...
export class MergeFiles extends CLIAction {

    static KEYS: string[] = ['-m', '--merge'];

    private _logger: Logger = new Logger('CLIAction:: MergeFiles');
            ...
            sequences(groups.map((files: string[]) => {
                return () => {
                    return new Promise((done) => {
                        ServiceElectron.IPC.request(new IPCMessages.CLIActionMergeFilesRequest({
                            files: files,
                        }), IPCMessages.CLIActionMergeFilesResponse).then((response: IPCMessages.CLIActionMergeFilesResponse) => {
