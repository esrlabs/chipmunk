...
import {
    DLTConnectionController,
    IConnectionOptions,
} from '../../controllers/connections/dlt.connection';
...
class ServiceDLTDeamonConnector implements IService {
    ...
    public init(): Promise<void> {
        return new Promise((resolve, reject) => {
            this._subscriptions.onSessionClosed =
                ServiceStreams.getSubjects().onSessionClosed.subscribe(
                    this._onSessionClosed.bind(this),
                );
            Promise.all([
                ServiceElectron.IPC.subscribe(
                    IPCMessages.DLTDeamonConnectRequest,
                    this._onDLTDeamonConnectRequest.bind(this),
                ).then((subscription: Subscription) => {
                    this._subscriptions.DLTDeamonConnectRequest = subscription;
                }),
                ServiceElectron.IPC.subscribe(
                    IPCMessages.DLTDeamonDisconnectRequest,
                    this._onDLTDeamonDisconnectRequest.bind(this),
                ).then((subscription: Subscription) => {
                    this._subscriptions.DLTDeamonDisconnectRequest = subscription;
                }),
                ServiceElectron.IPC.subscribe(
                    IPCMessages.DLTDeamonRecentRequest,
                    this._onDLTDeamonRecentRequest.bind(this),
                ).then((subscription: Subscription) => {
                    this._subscriptions.DLTDeamonRecentRequest = subscription;
                }),
                ServiceElectron.IPC.subscribe(
                    IPCMessages.DLTDeamonRecentDropRequest,
                    this._onDLTDeamonRecentDropRequest.bind(this),
                ).then((subscription: Subscription) => {
                    this._subscriptions.DLTDeamonRecentDropRequest = subscription;
                }),
                ServiceElectron.IPC.subscribe(
                    IPCMessages.DLTDeamonSaveRequest,
                    this._onDLTDeamonSaveRequest.bind(this),
                ).then((subscription: Subscription) => {
    ...
    public destroy(): Promise<void> {
        return new Promise((resolve) => {
            ...
            Promise.all(
                Array.from(this._connections.values()).map(
                    (connection: DLTConnectionController) => {
                ...
                .catch((error: Error) => {
                    this._logger.error(
                        `Fail to correctly disconnect all due error: ${error.message}`,
                    );
                    resolve();
                });
        });
    ...
    public saveAs(
        target: string,
        type: 'session' | 'file',
        sections: CommonInterfaces.IIndexSection[] = [],
        ...
        return new Promise((resolve, reject) => {
            this.getFileName()
                .then((filename: string | undefined) => {
                    ...
                    this._saver = indexer
                        .exportDltFile(target, type, filename, { sections: sections })
                        .then(() => {
    ...
    private _onDLTDeamonConnectRequest(
        request: IPCMessages.TMessage,
        response: (instance: IPCMessages.TMessage) => any,
    ) {
        const req: IPCMessages.DLTDeamonConnectRequest =
            request as IPCMessages.DLTDeamonConnectRequest;
            ...
            .then(() => {
                ...
                connection.once(DLTConnectionController.Events.connect, () => {
                    ...
                    ServiceElectron.IPC.send(
                        new IPCMessages.DLTDeamonConnectEvent({
                            id: req.id,
                            session: req.session,
                        }),
                    ).catch((error: Error) => {
                        this._logger.warn(
                            `Fail to notify front-end about connection due error: ${error.message}`,
                        );
                    });
                ...
                connection.once(DLTConnectionController.Events.disconnect, () => {
                    ...
                    ServiceElectron.IPC.send(
                        new IPCMessages.DLTDeamonDisconnectEvent({
                            id: req.id,
                            session: req.session,
                        }),
                    ).catch((error: Error) => {
                        this._logger.warn(
                            `Fail to notify front-end about disconnection due error: ${error.message}`,
                        );
                    });
                });
                // Lsiten for error
    ...
    private _onDLTDeamonDisconnectRequest(
        request: IPCMessages.TMessage,
        response: (instance: IPCMessages.TMessage) => any,
    ) {
        const req: IPCMessages.DLTDeamonDisconnectRequest =
            request as IPCMessages.DLTDeamonDisconnectRequest;
    ...
    private _onDLTDeamonRecentRequest(
        request: IPCMessages.TMessage,
        response: (instance: IPCMessages.TMessage) => any,
    ) {
        const stored: IStorageScheme.IConnectionOptions[] =
            ServiceStorage.get().get().recentDLTConnectorSettings;
        response(
    ...
    private _onDLTDeamonRecentDropRequest(
        request: IPCMessages.TMessage,
        response: (instance: IPCMessages.TMessage) => any,
    ) {
        ServiceStorage.get()
        ...
        response(new IPCMessages.DLTDeamonRecentDropResponse());
    }

    private _onDLTDeamonSaveRequest(
        request: IPCMessages.TMessage,
        response: (instance: IPCMessages.TMessage) => any,
    ) {
        ...
        this.saveAs(req.session, 'session')
            .then((filename: string | undefined) => {
                response(
                    new IPCMessages.DLTDeamonSaveResponse({
                        session: req.session,
    ...
    private _onSessionClosed(guid: string) {
        ...
        connection
            ...
            .catch((error: Error) => {
                this._logger.error(
                    `Fail to disconnect in scope of the session "${guid}" due error: ${error.message}`,
                );
            });
    }

    private _updateRecent(settings: IConnectionOptions) {
        let stored: IStorageScheme.IConnectionOptions[] =
            ServiceStorage.get().get().recentDLTConnectorSettings;
    ...
    private _exportSelection(
        ...
        return new Promise((resolve, reject) => {
            ...
            this.saveAs(session, 'session', sections)
                .then(() => {
                    resolve();
                })
                .catch((saveErr: Error) => {
    ...
    private _isExportPossible(
        session: string,
        request:
            | IPCMessages.OutputExportFeaturesRequest
            | IPCMessages.OutputExportFeatureCallRequest,
    ): boolean {
