...
void exports_chipmunk_parser_parser_get_render_options(
  ...
  parse_string_dup(&first_column.description, "First Column Description");
  first_column.width = 30;

  auto &second_column = columns.ptr[1];
  parse_string_dup(&second_column.caption, "Second Column");
  parse_string_dup(&second_column.description, "Second Column Description ");
...
bool exports_chipmunk_parser_parser_parse(
  ...
  exports_chipmunk_parser_parser_parse_return_free(ret->ptr);
  ret->ptr = (exports_chipmunk_parser_parser_parse_return_t *)malloc(
      // alignof(exports_chipmunk_parser_parser_parse_return_t),
      sizeof(exports_chipmunk_parser_parser_parse_return_t));
  ret->len = 1;
  auto &item = ret->ptr[0];
  item.consumed = data->len;
  item.value.is_some = true;
