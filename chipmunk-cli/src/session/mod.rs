    ...
    D: ByteSource,
{
    ...
    let mut out_file = if let Some(output) = output {
        ...
        for (_, item) in items {
            match item {
                parsers::MessageStreamItem::Item(parse_yield) => {
                    ...
                    if let Some(writer) = out_file.as_mut() {
                        writeln!(writer, "{repalced_msg_buffer}")
                            .context("Error while writing to output file")?;
                        if msg_count % 5000 == 0 {
                            println!(
                                "Parsing... {msg_count} message has been written to file sofar."
                            );
                        }
                    } else {
                        println!("{repalced_msg_buffer}");
                    }
                }
